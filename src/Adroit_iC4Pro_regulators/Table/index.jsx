...
import Header from "./regulatorExport";
import { MultiSelect } from 'primereact/multiselect';

import { Dropdown } from 'primereact/dropdown';
import oprtData from '../../Data/tempData/ic4pro_regulatorData.json';
import oprData from '../../Data/tempData/ic4pro_regulatorData.json';
import "./regulatorDataTable.css";
import "./regulatorColorBadges.css";
...
const Table = ({ data, selectedData, selectData, filter, grouping }) => {
  ...
  const operations = [...oprData.map(et=> et.ic4proRegulatorCode )];
...
const arrFilterRecordDateOffArray = getUniqueRecordDate(oprtData, 'ic4proRecordDate')
  ...
  const operationItemTemplate = (option) => {
    rowcolorRecord = oprData.filter(function (getRecord) {
      ...
      console.log("getRecord.ic4proRegulatorCode:",getRecord.ic4proRegulatorCode)
      return getRecord.ic4proRegulatorCode=== option
    }) 
    return (
      <span style={{background:rowcolorRecord.map(ee=>ee.ic4proFunctionColor),fontWeight: "bold"}}>{rowcolorRecord.map(ee=>ee.ic4proInstalledDate)}</span> 
